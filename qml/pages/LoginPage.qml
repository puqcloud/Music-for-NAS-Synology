...
Item {
    ...
    Flickable {
        ...
        Column {
            ...
            Rectangle {
                ...
                height: units.gu(6.2)
                radius: units.gu(3.1)
                color: isFormValid ? (signInMouse.pressed ? Theme.primaryDark : Theme.primary) : Theme.primaryLight
                border.color: "#ffffff"
            ...
            // Downloaded Songs (Offline Mode) Button
            Rectangle {
                id: offlineBtn
                width: parent.width
                height: units.gu(7)
                radius: units.gu(3.5)
                color: offlineMouse.pressed ? "#424242" : "#212121"
    ...
    Component.onCompleted: {
        ...
        if (savedUrl) serverInput.text = savedUrl;
    ...
    function doLogin() {
        ...
        // Clean protocol from server string for saving
        var cleanServer = rawServer.replace(/^https?:\/\//i, "");
        // Format Server URL for API
        var server = (isHttps ? "https://" : "http://") + cleanServer;
        ...
        SynoApi.login(server, user, pass, otp, function(err, data) {
            ...
            if (err) {
                if (err.code === 403 || err.code === 404 || err.code === 406) {
                ...
                } else if (err.code === 0 || err.code === -1 || err.code === -2 ||
                    ...
                    mainView.showErrorDialog(i18n.tr("Sign In"), err.message || i18n.tr("Account or password invalid. Please try again."));
                }
                return;
            }
            ...
            if (data && data.sid) {
                ...
                Storage.setSetting("serverUrl", cleanServer);
                Storage.setSetting("username", user);
                Storage.setSetting("password", ""); // Erase any previously saved password
                ...
                passInput.text = ""; // Never keep the password in UI memory
                otpInput.text = "";
