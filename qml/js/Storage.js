...
function getDatabase() {
    if (db) return db;
    try {
        db = Sql.LocalStorage.openDatabaseSync("MusicSynologyDB", "", "Storage for Music for NAS Synology", 200000);
...
// Wipes ALL application data: settings, credentials, download metadata,
// sync records and cache records. Leaves the DB schema intact.
function fullReset() {
    var database = getDatabase();
    if (!database) return false;
    try {
...
function clearAllDownloadsMetadata() {
    var database = getDatabase();
    if (!database) return false;
    try {
        database.transaction(function(tx) {
            tx.executeSql('DELETE FROM settings WHERE key LIKE ?', ['dl_%']);
        });
        return true;
    } catch(e) {
        console.error("Error clearing downloads metadata:", e);
        return false;
    }
}

// Library lists are cached with cover URLs that embed the old session id:
// wipe them when the session changes so stale credentials never linger.
function clearLibraryCache() {
    var database = getDatabase();
    if (!database) return false;
    try {
        database.transaction(function(tx) {
            tx.executeSql('DELETE FROM settings WHERE key LIKE ?', ['lib_cache_%']);
        });
        return true;
    } catch(e) {
        console.error("Error clearing library cache:", e);
        return false;
    }
}

function removeDownloadedSongMetadata(songId) {
    removeSetting("dl_title_" + songId);
    removeSetting("dl_artist_" + songId);
    removeSetting("dl_album_" + songId);
}
